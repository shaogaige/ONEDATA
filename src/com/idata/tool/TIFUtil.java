...
public class TIFUtil {
    ...
    public static void tif2Tiles(String tifFile, String outputDir, int minZoom, int maxZoom) throws FactoryException, TransformException {
        ...
        try 
        {
            ...
            bounds.addProperty("east", lngMax);
            bounds.addProperty("north", latMax);
            bounds.addProperty("south", latMin);
            bounds.addProperty("west", lngMin);
            ...
            Coordinate lngLatMax = JTS.transform(new Coordinate(lngMax, latMax), new Coordinate(), transform);
            Coordinate lngLatMin = JTS.transform(new Coordinate(lngMin, latMin), new Coordinate(), transform);
            lngMax = lngLatMax.y;
            latMax = lngLatMax.x;
            lngMin = lngLatMin.y;
            latMin = lngLatMin.x;
            JsonObject latlonbounds = new JsonObject();
            latlonbounds.addProperty("east", lngMax);
            latlonbounds.addProperty("north", latMax);
            latlonbounds.addProperty("south", latMin);
            latlonbounds.addProperty("west", lngMin);
            config.add("latlonbounds", latlonbounds);
            // 原始图像东西方向像素分辨率
            ...
            for (int zoom = minZoom; zoom <= maxZoom; zoom++) 
            {
                ...
                for (int row = tileRowMin; row <= tileRowMax; row++) 
                {
                    for (int col = tileColMin; col <= tileColMax; col++) 
                    {
                        ...
                        String pngPath = outputDir + "/" + zoom + "/" + col + "/" + row + ".png";
            ...
            config.addProperty("minzoom", minZoom);
            config.addProperty("maxzoom", maxZoom);
            config.addProperty("proj", "3857");
            config.addProperty("type", "image/png");
            File f = new File(tifFile);
            config.addProperty("name", f.getName());
            config.addProperty("scheme", "tms");
            config.addProperty("version", "1.0.0");
            FileUtil.writeFile(outputDir+"/config.json", config.toString().getBytes());
        } finally {
            // 释放并删除临时文件
            dataset.delete();
