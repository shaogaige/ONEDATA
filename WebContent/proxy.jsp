...
<%!
    ...
    private String addAdministrationToUri(String uri) {
    	//获取adminstration
        ...
        {
        	String[] arrSplitEqu = strSplit.split("[=]");
        	if(arrSplitEqu != null && arrSplitEqu.length >1)
        	{
        		if("administration".equalsIgnoreCase(arrSplitEqu[0]))
        		{
        			admin = arrSplitEqu[1];
        			break;
        		}
    	...
    	//根据参数替换内容
    	if(admin != null && !"".equalsIgnoreCase(admin))
    	{
    		String key = "bbox",value = "";
    		//获取空间范围
    		value = ProxyConfig.admins.get(admin.toLowerCase());
    		//不同服务类型
    		
    		//替换字符串
    		uri = uri.replaceAll("(administration=[^&]*)", key + "=" + value);
    	}
    	//返回新的请求串
    ...
    public static class ProxyConfig
    {
            ...
            if (configFile != null) {
                ...
                if (found){
                    ...
                    if (proxyConfigAttributes != null && !proxyConfigAttributes.isEmpty()){
                        ...
                        //-------------mycode------------------
                        p = Pattern.compile("<\\s*administrations\\s*>(.+?)<\\s*/\\s*administrations\\s*>", Pattern.MULTILINE | Pattern.DOTALL);
                        ...
                        if (found) {
                            ...
                            if (serverUrls != null && !serverUrls.isEmpty()) {
                                p = Pattern.compile("<\\s*administration (.+?)((<\\s*/\\s*administration\\s*)|/)>", Pattern.MULTILINE | Pattern.DOTALL);
                                m = p.matcher(serverUrls);
                                
                                while(m.find()){
                                    String server = m.group(1);
                                    String name = ProxyConfig.getAttributeWithRegex("name", server);
                                    String geo = ProxyConfig.getAttributeWithRegex("geometry", server);
						...
						p = Pattern.compile("<\\s*token\\s*>(.+?)<\\s*/\\s*token\\s*>", Pattern.MULTILINE | Pattern.DOTALL);
                        m = p.matcher(configFileStr);
                        found = m.find();

                        if (found) {
                            token_tianditu = m.group(1);
						}
        ...
        public static Map<String,String> admins = new HashMap<String,String>();
		
		public static String token_tianditu;
...
%><%
    ...
    try {
        ...
        //------------mycode-----------
        boolean hasAdministration = requestUri.contains("?administration=") || requestUri.contains("&administration=") || post.contains("?administration=") || post.contains("&administration=");
        if(hasAdministration)
        {
        	//has administration
        	requestUri = addAdministrationToUri(requestUri);
        }
		
		if(requestUri.contains("tianditu"))
		{
			requestUri += "&tk="+ProxyConfig.token_tianditu;
		}
